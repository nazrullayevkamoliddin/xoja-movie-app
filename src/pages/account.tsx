...
const Account = ({subscription}:AccountProps ) => {
  ...
  return (
    <>
      <Head>
        <title> Account Sozlamalari </title>
        <meta name="description" content="Accountni sozlash" />
      ...
      <div>
        ...
        <main className="mx-auto max-w-6xl px-5 pt-24 pb-12 transition-all md:px-10">
          <div className="flex flex-col gap-x-4 md:flex-row md:items-center">
            ...
            <div className="-ml-1 flex items-center gap-x-1.5">
              ...
              <p className="text-md font-semibold text-[#555]">
                Member since {moment(subscription.current_period_start * 1000).format('DD MM, yyyy')} 
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:border-x-0 md:border-t md:border-b-0 md:px-0 md:pb-4">
            <h4 className="text-lg text-[gray]"> Plan Details </h4>
            <div className="cols-span-2 font-medium">{subscription.plan.nickname}</div>
            <p className="cursor-pointer text-blue-500 hover:underline md:text-right"> Change Plan </p>
          </div>

          <div className="mt-6 grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:border-x-0 md:border-t md:border-b-0 md:px-0 md:pb-4">
            <h4 className="text-lg text-[gray]"> Settings </h4>
            <p className="col-span-3 cursor-pointe text-blue-500 hover:underline" onClick={logout}>Sign out all devices</p>
          </div>
        </main>
      </div>
    </>
